...
namespace se::cs::dialog::edit_spell_object_window {
	...
	void PatchDialogProc_AfterMove(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		settings.spell_object_window.x_position = xPos - 3;
		settings.spell_object_window.y_position = yPos - 26;
	}
	...
	LRESULT CALLBACK PatchDialogProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		const auto CS_SpellObjectDialogProc = reinterpret_cast<WNDPROC>(0x432AE0);
		auto vanillaResult = CS_SpellObjectDialogProc(hWnd, msg, wParam, lParam);
