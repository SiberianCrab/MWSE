...
namespace se::cs::dialog::edit_faction_window {
	...
	void setRedrawOnExpensiveWindows(HWND hWnd, bool redraw) {
		...
		SendDlgItemMessageA(hWnd, CONTROL_ID_REQUIREMENTS_FAVORED_SKILLS_LIST, WM_SETREDRAW, wParam, NULL);
		SendDlgItemMessageA(hWnd, CONTROL_ID_REQUIREMENTS_FAVORED_ATTR_1_COMBO, WM_SETREDRAW, wParam, NULL);
		SendDlgItemMessageA(hWnd, CONTROL_ID_REQUIREMENTS_FAVORED_ATTR_2_COMBO, WM_SETREDRAW, wParam, NULL);
		SendDlgItemMessageA(hWnd, CONTROL_ID_REQUIREMENTS_RANKS_STAT_LIST, WM_SETREDRAW, wParam, NULL);
		SendDlgItemMessageA(hWnd, CONTROL_ID_FACTION_REACTIONS_LIST, WM_SETREDRAW, wParam, NULL);
		SendDlgItemMessageA(hWnd, CONTROL_ID_FACTION_REACTIONS_FACTION_SEL_COMBO, WM_SETREDRAW, wParam, NULL);
	}
	...
	void PatchDialogProc_AfterInitialize(DialogProcContext& context) {
		...
		if constexpr (LOG_PERFORMANCE_RESULTS) {
			...
			log::stream << "Displaying faction data took " << timeToInitialize.count() << "ms" << std::endl;
		}
		...
		const auto x = settings.faction_window.x_position;
		const auto y = settings.faction_window.y_position;
	...
	void PatchDialogProc_AfterMove(DialogProcContext& context) {
		settings.faction_window.x_position = context.getMovedX() - 3;
		settings.faction_window.y_position = context.getMovedY() - 26;
	}

	LRESULT CALLBACK PatchDialogProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		DialogProcContext context(hWnd, msg, wParam, lParam, 0x4270B0);
	...
	void installPatches() {
		...
		genJumpEnforced(0x4034E5, 0x4270B0, reinterpret_cast<DWORD>(PatchDialogProc));
	}
}